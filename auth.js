...
function renderAuthNav() {
  ...
  if (currentUser) {
    navBox.innerHTML = `
      <a class="btn btn-light" href="profile.html">👤 Кабинет</a>
      <button class="btn btn-danger" onclick="logoutUser()">Выйти</button>
    `;
  } else {
    navBox.innerHTML = `
      <button class="btn btn-light" onclick="openAuth('login')">Войти</button>
      <button class="btn btn-light" onclick="openAuth('register')">Регистрация</button>
    `;
  }
}
...
function openAuth(mode) {
  ...
  document.getElementById("authTitle").innerText = mode === "register" ? "Регистрация" : "Вход";
